...
import {View, StyleSheet, Text, ScrollView, Pressable, Image, TouchableOpacity, FlatList, BackHandler, ActivityIndicator} from 'react-native';
...
const WatchAnimeScreen = ({route}) => {
    ...
    const FetchAnimeAndColor = async () => {
        try {
          ...
          const detailEpsAnime = await fetchEpisode(episodeId);
    ...
    <SafeAreaView style={{flex: 1, backgroundColor: '#000'}}>
      ...
      <ScrollView style={{flex: 1}} showsVerticalScrollIndicator={false}>
        ...
        <LinearGradient               
          ...
          style={{flex: 1}}
        >
            <View style={{width: '100%', paddingVertical: 10, flexDirection: 'row', overflow: 'hidden'}}>
                <Image style={{height: 100, width: 80, marginRight: 10, marginLeft: 10, borderRadius: 5}} source={{uri: data?.poster }} />
                <View style={{flex: 1, paddingRight: 10}}>
                    <Text style={{color: '#fff', fontFamily: "NotoSans_SemiCondensed-Bold", fontSize: 20}} numberOfLines={2}>{data?.title}</Text>
                    <Text style={{color: 'rgba(255,255,255,0.7)', fontFamily: "NotoSans_SemiCondensed-Regular", fontSize: 14}} numberOfLines={1}>{data?.releasedOn}</Text>
                </View>
            </View>
            <View style={{ width: "100%", paddingBottom: 10}}>
                <FlatList
                    ...
                    >
                        <Text style={{ color: "#fff", fontFamily: "NotoSans_SemiCondensed-Bold", fontSize: 12, }} numberOfLines={1}>
                        {item.title}
                        </Text>
                    </Pressable>
                    )}
            ...
            <View style={{width: '100%', flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 10}}>
                <Pressable 
                    ...
                    style={{width: '45%', paddingVertical: 7, justifyContent: 'center', alignItems: 'center', backgroundColor: data?.hasPrevEpisode ? "#1E293B" : "#64748B", opacity: data?.hasPrevEpisode ? 1 : 0.5, borderRadius: 30}}
                    onPress={() => {
                        if (data?.hasPrevEpisode && data?.prevEpisode) {
                            navigation.replace("WatchAnime", { episodeId: data.prevEpisode.episodeId });
                        }
                    }}
                >
                    <Text style={{color: '#fff', fontFamily: "NotoSans_SemiCondensed-Bold", fontSize: 15}} numberOfLines={1}>Prev Episode</Text>
                </Pressable>
                <Pressable
                    disabled={!data?.hasNextEpisode}
                    onPress={() => {
                        if (data?.hasNextEpisode) {
                            navigation.replace("WatchAnime", { episodeId: data.nextEpisode.episodeId });
                        }
                    }}
                    style={{
                        width: '45%',
                    ...
                    >
                    <Text style={{ color: '#fff', fontFamily: "NotoSans_SemiCondensed-Bold", fontSize: 15 }} numberOfLines={1}>
                        Next Episode
                    </Text>
                    </Pressable>
            </View>
            <View style={{width: '100%', flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, paddingBottom: 20}}>
                <ServerSelector serverData={data?.server} setVideoUrl={setVideoUrl} />
            </View>
            <View
                style={{
                width: '100%',
            ...
            <View style={{width: '100%', flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, paddingBottom: 20, paddingTop: 50}}>
